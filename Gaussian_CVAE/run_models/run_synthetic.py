...
def run_synthetic(args, X_train, C_train, Cond_indices_train, X_test, C_test, Cond_indices_test, 
    ...
    dataframe2 = {'epoch': [],'dimension':[], 'train_rcl_per_dim': [], 'train_kld_per_dim': [], 'test_rcl_per_dim': [], 'test_kld_per_dim': [], 'num_conds': []}
    ...
    for i in range(n_epochs):
        ...
        for j in range(len(train_rcl_per_cond)):
            ...
            dataframe['num_conds'].append(model_kwargs['x_dim']- j)
        ...
        for j in range(len(train_rcl_per_cond)):
            ...
            # print(this_cond_kld)
            for k in range(len(this_cond_rcl)):
                dataframe2['epoch'].append(i)
                dataframe2['dimension'].append(k)
                dataframe2['num_conds'].append(model_kwargs['x_dim']- j)
                dataframe2['train_rcl_per_dim'].append(this_cond_rcl[k].item())
                dataframe2['train_kld_per_dim'].append(this_cond_kld[k].item())
                dataframe2['test_rcl_per_dim'].append(this_cond_rcl_test[k].item())
...
def train(args, epoch, loss_fn, X_train, C_train, Cond_indices_train, batch_size, model, optimizer, gpu_id, model_kwargs):
    ...
    for j, i in enumerate(X_train):
        ...
        # print('here')
        # print(rcl_per_element.size(), kld_per_element.size())
        loss.backward()
        ...
        for jj, ii in enumerate(torch.unique(cond_labels)):
            ...
            # print(rcl_per_element[this_cond_positions])
            # print(torch.sum(rcl_per_element[this_cond_positions], dim = 0))
            # print(torch.cat([conds_rcl_1, torch.sum(rcl_per_element[this_cond_positions], dim = 0).cpu()], 0))
