...
class SyntheticDataset(Dataset):
    def __init__(self, num_batches, BATCH_SIZE, model_kwargs, shuffle=True, corr=False):
        """
        Args: 
        ...
        for j, i in enumerate(range(num_batches)):
            if corr is False:
                m = MultivariateNormal(torch.zeros(model_kwargs['x_dim']), torch.eye(model_kwargs['x_dim']))
            else:
                if j == 0:
                    corr_matrix = self.random_corr_mat(D = model_kwargs['x_dim'])
                    corr_matrix = torch.from_numpy(corr_matrix)
                m = MultivariateNormal(torch.zeros(model_kwargs['x_dim']).float(), corr_matrix.float())
